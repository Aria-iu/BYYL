...
pType copyType(pType src) {
    ...
    assert(p != NULL);
    p->cla = src->cla;
    assert(p->cla == BASIC || p->cla == ARRAY || p->cla == STRUCTURE ||
           p->cla == FUNCTION);
    switch (p->cla) {
        case BASIC:
...
void deleteType(pType type) {
    ...
    assert(type->cla == BASIC || type->cla == ARRAY ||
           type->cla == STRUCTURE || type->cla == FUNCTION);
...
boolean checkType(pType type1, pType type2) {
    ...
    if (type1->cla == FUNCTION || type2->cla == FUNCTION) return FALSE;
...
boolean checkTableItemConflict(pTable table, pItem item) {
    ...
    while (temp) {
        if (!strcmp(temp->field->name, item->field->name)) {
            if (temp->field->type->cla == STRUCTURE ||
                item->field->type->cla == STRUCTURE)
                return TRUE;
...
void Stmt(pNode node, pType returnType) {
    ...
    if (!strcmp(node->child->name, "Exp")) expType = Exp(node->child);
        ...
        if (!checkType(returnType, expType))
            pError(TYPE_MISMATCH_RETURN, node->linenum,
...
pType Exp(pNode node) {
    ...
    if (!strcmp(t->name, "Exp")) {
        ...
        if (funcInfo == NULL) {
            ...
            return NULL;
        } else if (funcInfo->field->type->cla != FUNCTION) {
            char msg[100] = {0};
